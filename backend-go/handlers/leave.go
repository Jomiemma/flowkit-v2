...
func CreateLeave(c *gin.Context) {
	...
	log.Printf("✅ Leave request created - Employee: %s, Type: %s, Days: %d, From: %s, To: %s",
		user.Email, leave.LeaveType, totalDays, fromDate.Format("2006-01-02"), toDate.Format("2006-01-02"))
...
func GetMyLeaves(c *gin.Context) {
	...
	for i, leave := range leaves {
		...
		// Update isEditable: false if HOD has started reviewing (not pending anymore)
		isEditable := leave.IsEditable && leave.HODApprovalStatus == "pending"
