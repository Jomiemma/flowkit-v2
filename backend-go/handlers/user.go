...
// GetRelievers gets potential relievers (employees in same department except current user)
func GetRelievers(c *gin.Context) {
	...
	err = config.UsersCollection.FindOne(ctx, bson.M{"_id": userID}).Decode(&currentUser)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"success": false,
