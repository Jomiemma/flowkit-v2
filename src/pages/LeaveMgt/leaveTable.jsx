...
const LeaveTable = () => {
  ...
  const handleLeaveCreated = () => {
    // Refresh the leave list after creating a new leave
    fetchLeaves();
  };
    ...
    return (
      <div className="mt-8 bg-white rounded-xl shadow-md p-6">
        <div className="flex items-center justify-center py-12">
          <div className="animate-pulse text-gray-400">Loading leave history...</div>
        </div>
      </div>
    );
  }
              ...
              >
                <td className="py-3 px-4 font-medium text-gray-700">
                  ...
                  {leave.otherLeaveType && (
                    <span className="text-xs text-gray-500"> ({leave.otherLeaveType})</span>
                  )}
                </td>
                ...
                <td className={`py-3 px-4 font-semibold ${getStatusColor(leave.status)}`}>
                  {leave.status}
                  {leave.status === "Pending" && (
                    <span className="text-xs text-gray-500"> (Stage {leave.stage})</span>
                  )}
                </td>
